...
// ============================================================
//  NEW METHODS FOR PIGEON VALIDATION - ADD TO app.js
//  Place these before window.onload = () => app.init();
// ============================================================

// ===== NEW: Load stickers with pigeon details =====
app.loadStickersForEventWithPigeonDetails = async function () {
  const select = document.getElementById("sticker-event-select");
  const eventCode = select ? select.value : "";
  if (!eventCode) {
    app.showModal({
  ...
  try {
    ...
    this._stickerState.isGenerating = true;
    const loadingDiv = document.getElementById("sticker-grid-container");
    if (loadingDiv) {
      loadingDiv.innerHTML = `
        <div class="sticker-loading">
    ...
    const res = await fetchWithAuth(
      `${API_URL}/admin/events/${eventCode}/codes-with-pigeons`,
    );
    ...
    if (!data.codes || data.codes.length === 0) {
      ...
      document.getElementById("sticker-total-count").textContent = "0 stickers";
      this._stickerState.isGenerating = false;
      return;
    }
    ...
    const stickers = data.codes.map((c) => ({
      ...
      code: c.code,
      pigeonId: c.pigeonId,
      ringNumber: c.ringNumber || "N/A",
      avatarId: c.avatarId || "",
      pigeonDisplay: c.pigeonName || c.ringNumber || "Unknown Pigeon",
    }));

    this._stickerState.stickers = stickers;
    this._stickerState.eventId = eventCode;
    this._stickerState.eventName = data.eventName;

    await this.renderStickersWithPigeonDetails(
      stickers,
      this._stickerState.widthMm,
    );
    this._stickerState.isGenerating = false;
  } catch (e) {
    console.error("Load stickers error:", e);
...
// ===== NEW: Render stickers with pigeon names =====
app.renderStickersWithPigeonDetails = async function (stickers, widthMm) {
  const container = document.getElementById("sticker-grid-container");
  if (!container) return;
  ...
  container.innerHTML = `
    <div class="sticker-loading">
      <div class="spinner"></div>
      <p>Rendering stickers... (${stickers.length} stickers)</p>
    </div>
  `;

  const canvasItems = [];
  const dpi = this._stickerState ? this._stickerState.dpi : 300;
  const heightMm = 10;

  for (let i = 0; i < stickers.length; i++) {
    const s = stickers[i];
    try {
      const canvas = await this.renderStickerCanvasWithPigeon(
        s.eventName || this._stickerState?.eventName || "Event",
        s.playerName || "Player",
        s.code,
        s.pigeonDisplay || s.ringNumber || "Pigeon",
        widthMm,
        heightMm,
        dpi,
      );
  ...
  if (this._stickerState) {
    this._stickerState.canvasCache = canvasItems;
  }
  ...
  for (const item of canvasItems) {
    ...
    pigeonDiv.className = "sticker-pigeon";
    pigeonDiv.textContent = `🕊️ ${data.pigeonDisplay || data.ringNumber || "Unknown"}`;
    pigeonDiv.style.cssText = "font-size: 11px; color: var(--text-muted);";
    wrapper.appendChild(pigeonDiv);
  ...
  container.appendChild(grid);
  document.getElementById("sticker-total-count").textContent =
    `${canvasItems.length} stickers (${stickers.length} pigeons)`;
};

// ===== NEW: Render individual sticker with pigeon name =====
app.renderStickerCanvasWithPigeon = async function (
  eventName,
  playerName,
  code,
  pigeonName,
  ...
  let testSize = fontSizeBottom;
  ctx.font = `${testSize}px sans-serif`;
  while (testSize > 8 && ctx.measureText(playerName).width > maxWidth) {
    testSize -= 1;
    ctx.font = `${testSize}px sans-serif`;
  }
  ctx.textAlign = "left";
  ctx.textBaseline = "middle";
  ctx.fillStyle = "#000000";
  ctx.fillText(playerName, leftX, centerY - 2);

  const pigeonLabel = pigeonName || "Pigeon";
  testSize = Math.round(fontSizeBottom * 0.85);
  ctx.font = `${testSize}px sans-serif`;
  while (testSize > 8 && ctx.measureText(pigeonLabel).width > maxWidth) {
    testSize -= 1;
    ctx.font = `${testSize}px sans-serif`;
  }
  ctx.textAlign = "right";
  ctx.textBaseline = "middle";
  ctx.fillStyle = "#2a7a62";
  ctx.fillText(`🕊️ ${pigeonLabel}`, rightX, centerY - 2);

  ctx.textAlign = "center";
  ctx.textBaseline = "bottom";
  ctx.fillStyle = "#999";
  ctx.font = `${Math.round(fontSizeBottom * 0.6)}px sans-serif`;
  ctx.fillText("MPFRC", widthPx / 2, bottomY + bottomHeightPx - 2);
...
// ===== NEW: Enhanced navigation to sticker generator =====
app.navigateToStickerGeneratorEnhanced = function (code) {
  if (code) {
    const select = document.getElementById("sticker-event-select");
    if (select) select.value = code;
  }
  ...
  setTimeout(() => {
    const select = document.getElementById("sticker-event-select");
    if (select && select.value) {
      this.loadStickersForEventWithPigeonDetails();
    }
  }, 300);
};

// ===== NEW: Verify pigeon code match =====
app.verifyPigeonCodeMatch = function (eventCode, pigeonId, code) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const data = await res.json();
      const match = data.codes.find((c) => c.code === code);
      resolve(match ? match.pigeonId === pigeonId : false);
    } catch (e) {
      reject(e);
    }
  });
...
// ===== NEW: Enhanced clock-in using the enhanced endpoint =====
app.clockInEnhanced = function () {
  const code = document.getElementById("clock-in-code").value.trim();
  if (!code) {
    this.showModal({
      title: "Missing Code",
  ...
  fetchWithAuth(`${API_URL}/clockin-enhanced`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      userId: this.currentUser.id,
    ...
    .then((data) => {
      ...
      const pigeonInfo = data.pigeonId
        ? `\n🕊️ Pigeon ID: ${data.pigeonId}`
        : "";

      this.showModal({
        title: "✅ Bird Clocked!",
        message: `${formattedDate}  ${formattedTime}\n📋 ${eventName} (MPFRC)\n📏 Air Distance: ${data.distance.toFixed(4)} KM\n⚡ Speed: ${data.speed.toFixed(4)} m/min${pigeonInfo}`,
        icon: "✅",
        iconColor: "#27ae60",
        buttonText: "OK",
      });
